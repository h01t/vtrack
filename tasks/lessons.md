...
- On blackbox, local CUDA training is canonical. Point Ultralytics `datasets_dir` at `/srv/ai/datasets` (or `VTRACK_DATASETS_DIR`), default train `--device` to `cuda`, and mirror published weights under `/srv/ai/checkpoints/vtrack` in addition to repo `models/`.
- Free the GPU before KITTI training on the 8 GiB 3060 Ti — a loaded NLLB service alone can occupy ~7 GiB and cause silent contention or OOM.
